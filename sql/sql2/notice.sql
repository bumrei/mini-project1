...
insert into notice(code, title, content, cdate) values(notice_seq.nextval, '게임 포인트 추가', '게임 단계별로 포인트를 설정하였습니다. ( 1단계: 10p, 2단계: 20p, 3단계: 30p ) 상점 기능은 앞으로 추가 예정입니다.', '21/06/28');


insert into notice(code, title, content, cdate) values(notice_seq.nextval, '이모티콘과 상점추가', '닉네임 앞에 설정할 이모티콘을 파는 상점을 오픈하였습니다. 게임에서 얻은 포인트로 구매 가능합니다. ( 1단계: 10p, 2단계: 20p, 3단계: 30p )', '21/07/01');

insert into notice(code, title, content, cdate) values(notice_seq.nextval, '게임 포인트 랜덤화', '각 단계별로 일정하던 포인트를 난수로 변경하였습니다. ( 1단계: 1~10p, 2단계: 11~20p, 3단계: 21~30p )', '21/07/07');
